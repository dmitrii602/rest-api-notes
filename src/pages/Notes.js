        ...
        <>
            ...
            <div className="w-full px-4 mb-4">
                <div className="block w-full bg-gray-800 rounded-lg h-10 outline-none p-2 flex">
                    ...
                    <input value={searchQuery} onChange={handleChange} type="text" className="flex-1 outline-none h-full bg-transparent ml-2 font text-gray-300" placeholder="Search" />
                </div>
            </div>
            <div className="px-4 mb-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {notes.filter(note => {
                    return note.title.toLowerCase().match(searchQuery.toLowerCase()) || note.body.toLowerCase().match(searchQuery.toLowerCase());
                }).map(note => (
